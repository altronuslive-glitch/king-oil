...
> **Поправка 11 сентября 2026.** В CS-Cart каждый объём — отдельный товар, и заказчик
> решил: чипс — **ссылка на соседний товар** (`<a class="volume" data-product-id>`), свой
> объём — подпись `<span class="volume is-active">`. Цена на месте не меняется, артикулов
> в каталоге нет. Габарит чипса тот же (28 / 30 px), см. [29-cscart-data-pass.md](29-cscart-data-pass.md).
